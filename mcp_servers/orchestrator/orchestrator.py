"""
Candidate Sample Orchestrator - Core Promptune optimization algorithm.

Coordinates evaluator and optimizers through iterative optimization.
...
async def optimize_prompt(
    ...
    """
    Run iterative optimization on a prompt.
    ...
    if verbose:
        logger.header("Promptune - Automated Prompt Optimization")
        ...
        logger.info(f"Sample width: {opt.sample_width}")
    ...
    for iteration in range(1, opt.max_iterations + 1):
        ...
        # Evaluate current candidate sample
        if verbose:
            logger.stage(Component.EVALUATOR, f"Evaluating candidates ({len(sample)} prompts)")
        ...
        best_prompt = sample[best_idx]
        best_eval = evaluated[best_idx][1]

        if verbose:
            logger.info(f"Best score in sample: {best_score:.0%}")
        ...
        iter_result = IterationResult(
            ...
            sample_prompts=sample.copy(),
            candidates_generated=0,
            candidates_evaluated=len(sample),
        )
        ...
        cross_poll = [best_prompt] if len(sample) > 1 else None

        for prompt_idx, (prompt, eval_result) in enumerate(evaluated):
            if verbose:
                logger.info(f"Processing candidate {prompt_idx + 1}/{len(evaluated)}")
            ...
            if "meta_prompt" in opt.optimizers:
                ...
                generated.extend(meta_candidates)
            ...
            if "few_shot" in opt.optimizers:
                ...
                generated.extend(few_shot_candidates)
            ...
            if "adversarial" in opt.optimizers:
                ...
                generated.extend(adv_candidates)
            ...
            if "example_augmentor" in opt.optimizers:
                ...
                generated.extend(aug_candidates)
            ...
            if "clarity_rewriter" in opt.optimizers:
                ...
                generated.extend(clarity_candidates)

        iter_result.candidates_generated = len(generated)

        # Evaluate only new candidates (reuse sample scores from above)
        eval_cache = {prompt: eval_result for prompt, eval_result in evaluated}
        new_to_eval = list(set(c for c in generated if c not in eval_cache))

        if verbose:
            logger.stage(
                Component.ORCHESTRATOR,
                f"Selecting {opt.sample_width} from top {opt.sample_width * 2} candidates ({len(new_to_eval)} new)",
            )
        ...
        # Select top K*2 candidates, then randomly choose K from those
        all_evaluated.sort(key=lambda x: x[1].score, reverse=True)
        top_pool = all_evaluated[: opt.sample_width * 2]
        if len(top_pool) > opt.sample_width:
            selected = random.sample(top_pool, opt.sample_width)
            selected.sort(key=lambda x: x[1].score, reverse=True)
        else:
            selected = top_pool
        sample = [e[0] for e in selected]
...
async def step(
    ...
    Returns:
        Dict with new candidates, scores, and stats
    """
    ...
    # Generate new candidates
    generated: list[str] = []
    cross_poll = [best_prompt] if len(candidates) > 1 else None
    ...
    # Evaluate all and select top K*2, then randomly choose K
    k = len(candidates)
    all_prompts = list(set(candidates + generated))
    ...
    top_pool = all_evaluated[: k * 2]
    if len(top_pool) > k:
        selected = random.sample(top_pool, k)
        selected.sort(key=lambda x: x[1].score, reverse=True)
    else:
        selected = top_pool
    new_candidates = [e[0] for e in selected]
    new_scores = [e[1].score for e in selected]
